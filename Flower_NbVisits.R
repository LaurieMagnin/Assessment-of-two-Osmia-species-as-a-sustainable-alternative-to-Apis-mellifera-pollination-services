...
visit_tree <- subset(visit_tree, visit_tree$Introduced_bee != 'control' )
#Exploring data 
...
########################################### Plot the number of flower per species APPLE ###########################
...
apple_visit_Selected$nb.of.flower <- as.numeric(apple_visit_Selected$nb.of.flower)
...
apple_visit_Selected_AV <-apple_visit_Selected %>%                                 # Group data
  group_by(Specie) %>%
  dplyr::summarize(gr_sum = mean(nb.of.flower), sd = sd(nb.of.flower), n = n(),
                   se = sd / sqrt(n)) %>% 
  as.data.frame()

########################################### Plot the number of flower per species CHERRY ###########################

cherry_visit<-subset(visit_tree, visit_tree$Tree%in%"cherry")
cherry_visit_Selected<-subset(cherry_visit,cherry_visit$Specie%in% c("A.mellifera", "B.terrestris","O.bicornis","O.cornuta"))
attach(cherry_visit_Selected)
names(cherry_visit_Selected)

cherry_visit_Selected$nb.of.flower <- as.numeric(cherry_visit_Selected$nb.of.flower)

P=ggplot(cherry_visit_Selected, aes(x=Specie, y=nb.of.flower)) + 
  geom_boxplot(fill="slateblue", alpha=1) + scale_y_continuous(name="number of flower visited",limits=c(0,20))+ xlab("Species")+
  theme(panel.border = element_blank(),panel.background = element_blank(),axis.line.y = element_line(colour = "grey"), panel.grid.major.y = element_line(size = 0.5, linetype = 'solid',colour = "grey"))
...
cherry_visit_Selected_AV <-cherry_visit_Selected %>%                                 # Group data
  group_by(Specie) %>%
  dplyr::summarize(gr_sum = mean(nb.of.flower), sd = sd(nb.of.flower), n = n(),
                   se = sd / sqrt(n)) %>% 
  as.data.frame()
