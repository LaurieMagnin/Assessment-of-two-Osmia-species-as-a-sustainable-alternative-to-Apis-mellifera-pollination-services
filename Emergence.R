...
Graph1<-ggsurvplot(model,linetype = "strata", data=emergence_cherry,ylab="Emergence", xlab="Day", fun = function(x) {1 - x},
                   legend.labs = c("O.bicornis_Ctrl", "O.bicornis_Field","O.cornuta_Ctrl","O.cornuta_Field"), palette = c("#9FC8C8","#298C8C", '#d8a6a6','#a00000'))

Graph1$plot <- Graph1$plot + scale_linetype_manual(values = c(2,1,2,1))+ theme(legend.text = element_text(size = 12))
print(Graph1)

ggsurvplot(model1,linetype = "strata", data=emergence_apple,ylab="Emergence", xlab="Day", fun = function(x) {1 - x},legend.labs = c("O.bicornis_Ctrl", "O.bicornis_Field","O.cornuta_Ctrl","O.cornuta_Field"))
...
emergence_cherry <- read.csv("~/working/Article_Osmia/emergence_cherry_censored.csv", sep=";")
